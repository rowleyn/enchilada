...
public class AdvancedClusterDialog extends JDialog implements ActionListener {
	...
	public AdvancedClusterDialog(JDialog frame) {
		...
		sampleBox = new JCheckBox("Random subsample fraction: ");
		p5.add(sampleBox);
		sampleField = new JTextField(3);
		sampleField.setEnabled(false);
		
		sampleBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				sampleField.setEnabled(sampleBox.isSelected());
			}
		});
...
//		stationaryBox = new JCheckBox("Hold initial centroids stationary", false);
//		p7.add(stationaryBox);
...
//		add(p7);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == okButton) {
			...
			// random sampling
			if (sampleBox.isSelected()) {
				text = sampleField.getText();
				((AbstractClusterDialog)parent).setSample(true,Double.parseDouble(text));
			}
			
//			boolean stn = stationaryBox.isSelected();
//			ClusterK.setStationary(stn);
